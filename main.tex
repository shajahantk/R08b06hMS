...
mimics key characteristics of movement in unicellular
organisms, including the run and tumble behavior of bacteria and the
locomotion of amoebas. They also sense and respond to the external conditions
mimicking electrotaxis, magnetotaxis, and chemotaxis
behaviors as in bacteria. For instance, a droplet containing 
Belousov-Zhabotinski reaction moves in a run and tumble pattern~\cite{suematsu2021spontaneous}, 
while other droplets can move in gradients of pH~\cite{lagzi2010maze}, 
salt concentration~\cite{cejkova2014dynamics}, or electric field \cite{hollo2021electric}.
  
\todo{Need to motivate a bit more. Intro is too short. So what? Why study them?}

The Belousov-Zhabotinsky (BZ) reaction has emerged as a classic system 
to study such movement patterns~\cite{steinbock1998radius, Heminghaus??}.  
The BZ reaction is a
...
change. The internal chemical oscillations in the BZ droplets
drives the mechanical motion, converting chemical energy to mechanical energy. Chemical oscillations in the BZ reaction can propagate in
space as chemical waves, creating circular and spiral
patterns~\cite{amrutha2022mechanism, amrutha2023theory,
  sebastian2024effect}. Many studies have shown that such a chemical
...
Many groups have investigated the dynamics and interactions of 
of BZ droplets in an oil medium
\cite{kitahata2011spontaneous, suematsu2016oscillation, kumar2021fast,
  suematsu2021spontaneous}. Their movement patterns in restricted 
geometries as in capillary tubes \cite{delgado2011coupled,
  li2014combined} and structured arrays
\cite{chang2018towards} were invetigated. 
They have observed that the chemical oscillations can be synchronised
...
indicate that one can tune the dynamics and interactions of these 
droplets by varying the concentration of sodium bromate.
We observed that as the frequency of chemical oscillations (???) in the
droplet increases, the interaction time between the BZ droplets
decreases. Furthermore, when two droplets with different oscillation 
frequencies interact, the interaction time is determined by 
the droplet with the higher activity.

\section{Methods}
We have used the aqueous droplets of BZ reaction mixture in immersed
in monoolein-squalane oil.  The chemical concentrations of
Belousov-Zhabotinsky stock solution is as follows: 0.96M malonic acid
(\ch{CH_2(COOH)_2}), 0.97M sodium bromide (NaBr), 0.5M sulphuric acid
(\ch{H_2SO_4}), and 25mM ferroin indicator solution
(\ch{[Fe(phe)_3]SO_4}) was used as a metal catalyst and indicator. We
varied the sodium bromate (\ch{NaBrO_3}) concentration from
0.48M-0.96M.  We mixed them using a magnetic stirrer with continuous
stirring until first chemical oscillation observed. The time required
to get the first oscillation is reduced with the sodium bromate
concentration. After the first oscillation, we placed 2$\mu$L of this
mixture into a 10mM monoolein-squalane oil medium inside a 5cm
diameter S-line Petri dish.  The experiments were conducted at room
temperature ($24^{\circ}$), maintaining consistent environmental
conditions throughout the experiments.

The movement of the BZ droplets were recorded
using a CMOS camera.
...
droplets recorded for the each frame. We have examined 
more than 300 droplets. A schematic diagram of the experimental setup is shown
in figure~\ref{fig:setup}.

\todo{How do we track the droplet? Specify the algorithm/method}
...
\begin{figure}[H]
	...
	\caption{\textbf{Experimental setup:} Schematic diagram of the  
		experimental setup.
		The BZ droplet was placed inside a Petri dish with a
...
\todo[inline]{Include a brief description of BZ reaction mechanism and
  about color oscillations. If needed a schematic can be added to
  figure 2.}  The Belousov-Zhabotinsky (BZ) reaction is a chemical
process characterized by cyclic oscillations. These oscillations can
be seen within a BZ droplet, displaying red and blue colors that
correspond to the oxidation states of a metal catalyst. When metal
catalyst is in its oxidized state ($Fe^{4+}$), the BZ reaction
exhibits a blue color, while in its reduced state ($Fe^{3+}$), it
appears red. This phenomenon allows us to observe the chemical
oscillations within the BZ droplet, as visible in figure \ref{fig:BZ
  Drop chemical wave}.



The Belousov-Zhabotinsky reaction is a complex process that consists
of three main stages, each involving numerous sub-reactions. In the
first stage, there is a continuous consumption of bromide (\ch{Br^-})
ions. Once the concentration of these ions drops below a certain
level, the second stage begins. During the second stage, the metal
catalyst undergoes oxidation, and the reaction turns blue. The third
stage involves a reduction of the metal catalyst, leading to a
significant increase in the concentration of \ch{Br^-} ions. As the
levels of \ch{Br^-} ions rise sufficiently, the cycle restarts with
the first stage. The third stage essentially acts as a reset for the
entire reaction \cite{field1974oscillations}. This process can be
visualized through the BZ droplet, as shown in figure \ref{fig:BZ
  Droplet}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{Figurs/Picture2.png}
	\caption{\textbf{Belousov zhabotinsky Reaction Droplet:} (a)
          It is the illustration of the self-propelled BZ droplet and
          mechanism of Belousov-Zhabotinsky chemical reaction inside
          the droplet. }
...
A small droplet containing 2$\mu L$ of the Belousov-Zhabotinsky
reagents is placed in the Petri dish inside the oily medium. The
dynamics of chemical oscillation and the subsequent movement of the
droplet can be controlled by the bromate concentration
???. Figure~\ref{fig:activity}~a shows the dynamics of chemical waves
within the droplet \todo{please produce a sequence of images}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\linewidth]{Figurs/111.png}
	\caption{\textbf{Belousov zhabotinsky Reaction in Droplet:}
          (a) It is the illustration of the self-propelled BZ droplet
          and mechanism of Belousov-Zhabotinsky chemical reaction
          inside the droplet. }
	\label{fig:BZ Drop chemical wave}
\end{figure}

The average light intensity inside the droplet varies periodically inside 
the droplet and we plot the same in Figure~\ref{fig:activity}~b. The 
movement of the droplet is tracked as discussed in the Method, and the 
trajectory of the droplet is shown in Figure~\ref{fig:activity}~c.
From this time series we calculate the frequency of the oscillation 
inside the droplet. From the trajectory we estimate the velocity of the 
droplet. We also characterise the movement as ballistic or random by
counting number of direction changes as the droplet moves.
\todo{Describe how do you calculate the direction change?}
...
We investigate the characteristics of the movement of BZ droplets as
we vary sodium bromate concentrations.  As the concentration of sodium
bromate increases, the BZ droplet’s trajectory becomes increasingly
complex (see figure~\ref{fig:activity}~(a-c)).  With higher sodium
bromate concentrations in the BZ droplet, the reaction rate increases,
resulting in a rise in the frequency of chemical oscillations within
the droplet. We determine the oscillation frequency from the average
blue color intensity of the BZ droplet. We consider the difference in
time between consecutive highest average blue color intensity as a
period of oscillation in the BZ droplet.  Figure~\ref{fig:activity}(d)
shows that the oscillation frequency is increased with sodium bromate
...
concentration. Figure~\ref{fig:activity}(d-f) shows that the frequency
of chemical oscillations inside BZ droplet, directional fluctuations,
speed of the BZ droplet increases with sodium bromate concentration.
This confirms that BZ droplets exhibit higher activity at higher
sodium bromate concentrations.

When several BZ droplets are placed in a Petri dish BZ droplets,
different droplets move in different directions.  But we observe that
droplets often discover other droplet, come closer to it, and move
around or moved parallel for some time, and then move away (see
figure~\ref{Inter-Droplet Distance}).  The circular motion of one
droplet around its pair is seen when the inter-droplet distance (the
distance between the centers of droplets) was 2.5mm or less.\todo{Is
  this true irrespective of the size of the droplets?}  (See
supplementary videos). The bromo-mono-olein layer that forms around
each droplet in the squalein-monolein medium can prevent the droplets
from merging~\cite{thutupalli2011bilayer}.

We charatcterised this pairwise interaction by measuring the total time 
in which the two droplets stay in close proximity. The 
interaction time is measured as the time interval during which
the inter-droplet distance is below 2.5~mm (figure~\ref{dropletDistance}).
...
concentration. We can observe that in Figure
...
another. We observed three distinct types of dynamics based on how the
droplets approach each other. The droplets can either arrive from
opposite directions, move in parallel, or have a larger droplet trail
behind a smaller one. Larger BZ droplet has the higher speed than the
smaller BZ droplet. When a small droplet approaches from the opposite
direction, or when both droplets move parallel while maintaining a
minimum separation, the interaction time is shorter than when a larger
droplet follows a smaller one. In the case where the large droplet
trails the small one, its speed is suppressed by the smaller droplet,
leading to a sustained interaction for long time (figure
\ref{Different_size}).
\begin{figure}[H]
    \centering
    ...
    \caption{\textbf{Interaction Time of Different-size Droplet
      ...
      different-sized droplets with interaction time. The interaction a and b are the interaction when droplets arrive from opposite directions, or
...
typically less than 50 seconds, as we can observe in Figure
\ref{fig:Big-Small}. Conversely, the interaction time exceeds 50
seconds when a larger droplet trails a smaller one. When the smaller
droplet follows the larger one, the interaction time is
negligible. The larger droplet is moving at a higher speed. When a
smaller droplet follows the larger droplet, the larger droplet easily
moves away from interaction and The interaction was not sustained. We
have recorded a total of 53 interactions for the different-sized
droplet interactions. We observed 22 percent parallel droplet
motion. In 45 percent of the interactions, droplets arrived from
opposite directions, while in 32 percent of the interactions, a
pattern of falling droplets followed the interaction. Additionally,
when two droplets of equal size interact, the time of interaction
remains consistent regardless of their approach sides. The collective
interaction of the BZ droplets were noted within a 2.5 mm or less
distance between them, Specifically for BZ droplets with a diameter of
1.75 mm.
%\begin{figure}[H]
%  \centering
...
\section{Discussion}
\todo{Please try to explain our results- why the droplets come closer
  and stay together and move away. What happens as bromine
  concentration increases. Each new statement should have citations.}
...
Sodium bromate is source \ch{Br^-} ions in the BZ reaction. At higher
concentrations of sodium bromate, the BZ reaction produced a large
number of \ch{Br^-} ions. When the sodium bromate concentration
increases in the BZ droplet, the rate of bromination of monoolein
increases around the surface of BZ droplet, and as a result, the speed
of the BZ droplet increases \cite{suematsu2021spontaneous}.


In the interaction of two BZ droplets, while moving in different
direction two droplets comes together and interact. How they detect
each other from the long distance still it is unclear. When the two BZ
comes close then they interact, thair interaction sustain until the
surrounding interfacial tension is reduced sufficiently. After the
sufficient reduction of interfacial tension around both the BZ
droplets, move away from each other. When the sodium bromate
concentration increased in the BZ droplets, the rate of reduction in
interfacial tension around BZ droplets increased due to bromination of
surfactant monoolein from the oil. As a result, the interaction time
of the BZ droplet decreased with the sodium bromate concentration.
